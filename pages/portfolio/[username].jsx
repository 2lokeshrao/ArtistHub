...
export default function PortfolioPage() {
  ...
  const [cart, setCart] = useState([]); // MULTI-SERVICE CART
  ...
  // CALCULATION LOGIC FOR CART
  const totalCartValue = cart.reduce((total, item) => total + Number(item.price), 0);
  const advance = Math.round(totalCartValue * 0.3);
  const upiLink = profile ? `upi://pay?pa=${profile.upi_id}&pn=${encodeURIComponent(profile.full_name)}&am=${advance}&cu=INR&tn=BookingCart` : '';
  ...
  const addToCart = (service) => {
    ...
    setStep(2); // Go to checkout section
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      await createBooking({ 
        ...
        service_id: cart[0].id, // Primary service link
        client_name: form.name, 
        client_phone: form.phone, 
        booking_date: form.date, 
        total_price: totalCartValue, 
        note: `Services: ${serviceNames} | Advance: ${advance} | UTR: ${form.utr}` 
      });
      setSuccess(true);
      openWhatsApp(profile.phone, { 
        ...
        advance, 
        pending: totalCartValue - advance, 
  ...
  return (
    <main className="min-h-screen bg-[#1A1A1A] text-white font-sans selection:bg-[#D4B996] selection:text-black">
      ...
      <div className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-50 bg-[#1A1A1A]/80 backdrop-blur-md border-b border-white/5">
        ...
        <button onClick={() => bookingRef.current?.scrollIntoView({ behavior: 'smooth' })} className="px-5 py-2 bg-[#D4B996] rounded-full text-[10px] font-black uppercase tracking-widest text-black flex items-center gap-2">
            Cart ({cart.length})
        </button>
      </div>
      ...
      <section className="mt-16 px-6">
        ...
        <div className="space-y-6">
          {services.map(s => {
            const isInCart = cart.find(item => item.id === s.id);
            return (
              <div key={s.id} className={`bg-[#1F1F1F] rounded-[32px] p-6 border transition-all ${isInCart ? 'border-[#D4B996] shadow-[0_0_20px_rgba(212,185,150,0.1)]' : 'border-[#D4B996]/10'}`}>
                ...
                <p className="text-xs text-white/50 leading-relaxed mb-6 font-medium">{s.description || "Premium service with professional finish."}</p>
                <button 
                  onClick={() => isInCart ? removeFromCart(s.id) : addToCart(s)}
                  className={`w-full py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isInCart ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-white/5 border border-[#D4B996]/30 text-[#D4B996]'}`}
                >
      ...
      {/* CHECKOUT DATES & MULTI-SERVICE TOTAL */}
        ...
        <div className="bg-[#1A1A1A] rounded-[40px] p-6 border border-white/5 shadow-2xl">
          {success ? (
            <div className="text-center py-10 animate-fadeIn">
              ...
              <button onClick={() => window.location.reload()} className="mt-8 text-[10px] uppercase font-black tracking-[0.3em] text-[#D4B996] border-b border-[#D4B996]">Back Home</button>
            </div>
          ) : (
            <div className="space-y-8">
              ...
              {cart.length === 0 ? (
                <div className="text-center py-10 border border-dashed border-white/10 rounded-3xl">
                  <p className="text-white/20 text-[10px] uppercase tracking-widest">Cart is Empty. Add services above.</p>
                </div>
              ) : (
                <>
                  {/* Cart Items Summary */}
                  <div className="space-y-3">
                    {cart.map(item => (
                      <div key={item.id} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                        ...
                        <span className="text-xs text-[#D4B996]">{formatINR(item.price)}</span>
                      </div>
                    ))}
                    <div className="flex justify-between p-4 border-t border-[#D4B996]/30 mt-4">
                        <span className="text-sm font-black text-[#D4B996]">TOTAL AMOUNT</span>
                        <span className="text-sm font-black text-white">{formatINR(totalCartValue)}</span>
                    </div>
                  </div>

                  {step === 2 && (
                    <div className="animate-slideUp">
                      <p className="text-[10px] text-white/40 uppercase text-center tracking-[0.3em] mb-4 font-bold">Pick Your Checkout Date</p>
                      <BookingCalendar busyDates={busyDates} selectedDate={form.date} onSelect={d => setForm({ ...form, date: d })} />
                      <button disabled={!form.date} onClick={() => setStep(3)} className="w-full mt-8 py-4 bg-[#D4B996] text-black font-black rounded-2xl uppercase text-[11px] tracking-widest shadow-xl">Confirm Date & Continue</button>
                    </div>
                  )}

                  {step === 3 && (
                    <div className="space-y-4 animate-slideUp">
                      <p className="text-[10px] text-white/40 uppercase text-center tracking-[0.3em] mb-4 font-bold">Billing Details</p>
                      <input placeholder="Enter Full Name" className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl outline-none text-sm font-bold" onChange={e => setForm({ ...form, name: e.target.value })} />
                      <input placeholder="WhatsApp Number" className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl outline-none text-sm font-bold" onChange={e => setForm({ ...form, phone: e.target.value })} />
                      <button disabled={!form.name || !form.phone} onClick={() => setStep(4)} className="w-full py-4 bg-[#D4B996] text-black font-black rounded-2xl uppercase text-[11px] tracking-widest shadow-xl">Proceed to Final Payment</button>
                    </div>
                  )}

                  {step === 4 && (
                    <div className="space-y-6 animate-slideUp text-center">
                      <div className="p-6 bg-[#D4B996]/5 rounded-3xl border border-[#D4B996]/30">
                        <p className="text-[10px] uppercase text-white/40 mb-2 font-bold tracking-widest italic text-center">Order Total: {formatINR(totalCartValue)}</p>
                        <p className="text-4xl font-black text-[#D4B996] tracking-tighter mb-6">{formatINR(advance)}</p>
                        <a href={upiLink} className="inline-block bg-[#5f259f] text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg">🟣 Open UPI Apps</a>
                      </div>
                      ...
                      <input placeholder="Enter 12-Digit Transaction ID" className="w-full p-4 bg-white/5 border border-[#D4B996]/30 rounded-2xl text-center outline-none text-sm font-black" onChange={e => setForm({ ...form, utr: e.target.value })} />
                      <button onClick={handleBookingSubmit} disabled={!form.utr || bookingLoading} className="w-full py-4 bg-[#D4B996] text-black font-black rounded-2xl uppercase text-[11px] tracking-widest shadow-xl">
                        {bookingLoading ? 'Securing Orders...' : 'Confirm All & Block Date'}
                      </button>
                    </div>
                  )}
                </>
