...
export async function upsertProfile(userId, payload) {
  ...
  const { data, error } = await supabase
    ...
    .upsert({ 
      ...payload, 
      id: userId,       // Yeh column RLS policy se match karega
      user_id: userId   // Yeh backup ke liye
    }, { 
      onConflict: 'id'  // Matches primary key
    })
    .select()
    .single();

  if (error) {
    console.error("Supabase RLS/Upsert Error:", error.message);
    throw error;
  }
  return data;
}
