...
/** Upsert profile */
/** Upsert profile - Fixed version */
export async function upsertProfile(userId, payload) {
  const { data, error } = await supabase
    .from('profiles')
    .upsert({ 
      ...payload, 
      id: userId,       // 'id' को प्राइमरी की की तरह भेजें
      user_id: userId   // 'user_id' को भी साथ रखें
    }, { 
      onConflict: 'id'   // अब यह 'id' के आधार पर मैच करेगा जो हमेशा यूनिक होती है
    })
    .select()
    .single();
